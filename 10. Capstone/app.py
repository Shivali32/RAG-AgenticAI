...
SYSTEM_PROMPT = """
You are Connoisseir Companion, an AI assistant with access to a database
of California restaurants.

You can use the following tools:
- get_restaurant_info: Look up specific restaurants by name.
- recommend_by_vibe: Find restaurants that match a particular mood or atmosphere.
...
Provide clear and helpful responses based on the restaurant information returned
by the tools.
"""
...
with gr.Blocks(title="Connoisseur Companion") as demo:
    ...
    btn1.click(
        handle_chat,
        [
            gr.State("Find me some moody restaurants"),
            chatbot
        ],
        [chatbot]
    )
    ...
    btn2.click(
        handle_chat,
        [
            gr.State("Tell me about Iron & Embers"),
            chatbot
        ],
        [chatbot]
    )
